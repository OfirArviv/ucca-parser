...
class UCCA_Parser(torch.nn.Module):
    def __init__(self, vocab, args, pre_emb=None, use_projections=False):
        ...
        if args.type == "chart":
            ...
            self.span_parser = Topdown_Span_Parser(
                ...
                drop=args.ffn_drop,
                norm=False,
                projection_dim=args.projection_dim if use_projections else 0
            )
    ...
    def parse(self, subword_idxs, subword_masks, token_starts_masks, lang_idxs, word_idxs, pos_idxs, dep_idxs, ent_idxs, ent_iob_idxs, passages, projections, trees=None, all_nodes=None, all_remote=None):
        ...
        if self.training:
            span_loss = self.span_parser.get_loss(spans, sen_lens, trees, projections)
            remote_loss = self.remote_parser.get_loss(spans, sen_lens, all_nodes, all_remote)
            return span_loss, remote_loss
        else:
            predict_trees = self.span_parser.predict(spans, sen_lens, projections)
    ...
    def load(cls, vocab_path, config_path, state_path):
        ...
        network = cls(vocab, config.ucca, use_projections=config.ucca.projections_path is not None)
