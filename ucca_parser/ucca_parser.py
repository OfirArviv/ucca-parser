...
class UCCA_Parser(torch.nn.Module):
    ...
    def parse(self, subword_idxs, subword_masks, token_starts_masks, lang_idxs, word_idxs, pos_idxs, dep_idxs, ent_idxs, ent_iob_idxs, passages, trees=None, all_nodes=None, all_remote=None, projections=None):
        if not isinstance(projections[0], Instance_2):
            projections = None
        if projections:
            for passage, projection in zip(passages, projections):
                assert passage.ID == projection.passage.ID
